/*************************************************************
 * @FilePath: /Cpp/ota/syntax/xmacro_list.hpp
 * @Brief: X-Macro "单一数据源" 列表定义（多个例程共用）
 *
 * 真实工程里同样的写法出现在：
 *   - src/ota/state_machine/state_machine.hpp  OTA_STATE_MACHINE_LIST
 *   - src/ota/state_machine/state_machine.hpp  UPGRADE_TYPE_LIST
 *   - src/ota/task/task.hpp                    OTA_TASK_TYPE_LIST
 *   - src/utils/[子模块]/error.hpp             OTA_ERROR_CODE_LIST
 *
 * 格式统一为：X(枚举名, 英文描述, 中文描述, 数值)
 ***************************************************************/
